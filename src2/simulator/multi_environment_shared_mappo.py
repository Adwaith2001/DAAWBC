"""
multi_environment_shared_mappo.py
TRUE shared auction environment for MAPPO.
...
class MultiRTBEnvironmentSharedMAPPO:
    ...
    def __init__(
        ...
        self.global_dim    = state_dim * self.num_agents
        ...
        print(f"Global state dim : {self.global_dim} "
              f"({self.num_agents} agents x {self.state_dim})")
        self.reset()
    ...
    def step(self, pctr_thresholds):
        ...
        if self.ptr >= self.n:
            return (
                ...
                np.zeros(self.global_dim, dtype=np.float32),
                rewards, True
            )
